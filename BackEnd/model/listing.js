...
var listingDB = {
    ...
    deleteListing: function (userid,affected, id, callback) {
        var conn = db.getConnection();
        if(userid != affected){
            return callback("Not authorized to delete this listing", null)
        }
        conn.connect(function (err) {
            if (err) {
                console.log(err);
                ...
                var sql = `delete from listings where id=${id} and fk_poster_id = ${affected}`;
